...
goldr_bioage <- function(d4, var, feature_selection = FALSE, 
  ...
  cv_fit <- glmnet::cv.glmnet(x = x_data, y = y_data, 
                               ...
                               alpha = alpha_val,
                               nfolds = nfolds)
  dd=data.frame(hhat=predict(cvfit,x_data),
                age=data[,"age"])
  ...
  cv_fit2 <- glmnet::cv.glmnet(x = x_data, y = resi, 
                               family = family, 
                               alpha = alpha_val,
                               nfolds = nfolds)
  yhat=predict(cvfit2,x_data)
...
goldrl_bioage <- function(d4, var, feature_selection = FALSE, 
                       selection_method = "lasso", alpha = 1, 
                       nfolds = 10, family = "cox") {
