...
gold_bioage <- function(d4, var, feature_selection = FALSE,
  ...
  biomarker_vars <- make.names(biomarker_vars)
  ...
  if (feature_selection && length(biomarker_vars) > 1) {
    ...
    # No feature selection or not enough biomarkers -> use all variables
    selected_vars <- var
    if (feature_selection && length(biomarker_vars) <= 1) {
  ...
  # Model 1: Age only
  # --- ADD missing Model 1 fit (was missing in original) ---
  ...
  result <- list(
    ...
    model2_coef = coef2,
    coefficients = coef21,
    GOLDBioage = data$GOLDbioage,
    residual=data$GOLDbioage-data$age,
    risk=risk2
  )
  return(result)
}
